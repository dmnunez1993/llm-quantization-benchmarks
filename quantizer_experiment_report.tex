...
\begin{enumerate}
    ...
    \item \textbf{Final experiment}: the final number of paired blocks was chosen using the pilot-based sample-size calculation described below. The final sample covered 70 weight blocks from 26 distinct down-projection layers.
\end{enumerate}
...
Table~\ref{tab:samplesize} shows the pilot-based sample-size calculation on randomly sampled Qwen3-0.6B weight blocks. The largest required sample size was 70 observations per algorithm, coming from the SQNR metric under the two-level symmetric scenario. Therefore, the final experiment used 70 paired blocks.
...
\midrule
SQNR bits & Two levels symmetric & 0.188140 & 0.10000 & 70 \\
SQNR bits & One versus rest & 0.188140 & 0.10000 & 53 \\
Quantize seconds & Two levels symmetric & 0.000148 & 0.00100 & 2 \\
Quantize seconds & One versus rest & 0.000148 & 0.00100 & 2 \\
Dequantize seconds & Two levels symmetric & 0.000073 & 0.00005 & 42 \\
Dequantize seconds & One versus rest & 0.000073 & 0.00005 & 32 \\
\bottomrule
\end{tabular}
\end{table}
...
The final experiment collected 70 paired Qwen3-0.6B weight blocks for each algorithm. These blocks were randomly sampled from 26 distinct MLP down-projection layers. Figure~\ref{fig:boxplots} shows the distributions of the main metrics.
...
\midrule
Leech Lattice Vector Quantization & 1.940660 & 0.069050 & [1.924196, 1.957125] & 0.035581 \\
Uniform scalar 2-bit & 1.414538 & 0.221851 & [1.361640, 1.467437] & 0.156836 \\
QTIP bitshift & 1.352473 & 0.237495 & [1.295844, 1.409101] & 0.175600 \\
\bottomrule
\end{tabular}
}
\end{table}
...
\midrule
Uniform scalar 2-bit & 0.000084 & 0.000052 & [0.000072, 0.000097] & 0.000044 & 0.000002 & [0.000044, 0.000044] \\
Leech Lattice Vector Quantization & 0.001353 & 0.000254 & [0.001292, 0.001413] & 0.000180 & 0.000086 & [0.000160, 0.000201] \\
QTIP bitshift & 0.008185 & 0.000118 & [0.008157, 0.008213] & 0.000092 & 0.000021 & [0.000087, 0.000097] \\
\bottomrule
\end{tabular}
}
\end{table}
...
\midrule
SQNR bits & Repeated-measures ANOVA & 290.95 & $3.16 \times 10^{-50}$ \\
SQNR bits & Friedman & 103.63 & $3.14 \times 10^{-23}$ \\
Quantize seconds & Repeated-measures ANOVA & 49431.78 & $8.96 \times 10^{-198}$ \\
Quantize seconds & Friedman & 140.00 & $3.98 \times 10^{-31}$ \\
Dequantize seconds & Repeated-measures ANOVA & 127.23 & $4.79 \times 10^{-32}$ \\
Dequantize seconds & Friedman & 136.11 & $2.77 \times 10^{-30}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{SQNR bits}: Leech Lattice Vector Quantization $>$ Uniform and Leech Lattice Vector Quantization $>$ QTIP. Uniform had a higher mean than QTIP, but that pairwise difference was not significant after Bonferroni correction.
...
Eight of the nine pairwise comparisons were significant after Bonferroni correction. The adjusted alpha was 0.005556. The only non-significant comparison was Uniform scalar 2-bit versus QTIP bitshift for SQNR bits.

\subsection{Normality Check}

Table~\ref{tab:assumptions} summarizes the Shapiro-Wilk checks on repeated-measures ANOVA residuals. SQNR and quantization-time residuals passed the normality check, but dequantization-time residuals did not. Therefore, the repeated-measures ANOVA result for dequantization time should be interpreted with caution. The Friedman tests are important because they corroborate the same global conclusion without relying on normal residuals.
...
\midrule
SQNR bits & Shapiro-Wilk residual normality & 0.3644 & Yes \\
Quantize seconds & Shapiro-Wilk residual normality & 0.0946 & Yes \\
Dequantize seconds & Shapiro-Wilk residual normality & $8.31 \times 10^{-22}$ & No \\
\bottomrule
\end{tabular}
\end{table}
...
The results show that there is no single best algorithm across all metrics for Qwen3-0.6B weight blocks. Leech Lattice Vector Quantization is the best choice when reconstruction quality is the priority. It achieved the highest SQNR and was significantly better than both scalar quantization and QTIP bitshift. However, this quality comes with higher runtime cost. Uniform scalar quantization was much faster in both quantization and dequantization, but its reconstruction quality was lower. QTIP bitshift did not improve SQNR in this configuration: its SQNR mean was slightly below uniform scalar quantization, and the SQNR difference between those two methods was not significant after correction. QTIP did, however, dequantize faster than Leech Lattice Vector Quantization.
...
The Shapiro-Wilk check is an important limitation. Dequantization residuals violate the normality assumption, so the repeated-measures ANOVA result for dequantization time should be interpreted cautiously. For this reason, the Friedman test is treated as a robustness check. Since both ANOVA and Friedman agree that the algorithms differ, the main conclusion is stable. However, the exact ANOVA p-value for dequantization time should not be over-interpreted.

\section{Conclusion}

This experiment compared three 2-bit quantization algorithms on actual Qwen3-0.6B weight blocks under a controlled paired design. The final sample size was calculated from a random multi-layer weight-block pilot study, resulting in 70 paired observations.

Leech Lattice Vector Quantization produced the best reconstruction quality, with SQNR mean 1.941 bits. Uniform scalar quantization was the fastest method, with mean quantization time 0.000084 seconds and mean dequantization time 0.000044 seconds. QTIP bitshift did not dominate either quality or runtime in this experimental configuration.
